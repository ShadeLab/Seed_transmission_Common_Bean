...
## Rarefaction curve figure

The non-rarefied dataset was used to preserve all rare taxa
...
#ggsave(file="/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/rarefaction_curve_NEW.png", plot=rarecurve_plot, width=8, height=5, units="in")
```
...
seed_phyloseq_decontam <- readRDS(file="/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/Seed_transmission_Common_Bean/R_Analysis_Files/seed_phyloseq_decontam_updatemeta.rds")

# Add tree to phyloseq object
seed_tree <- read_tree(treefile = "/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/Seed_transmission_Common_Bean/R_Analysis_Files/tree.nwk")
...
seed_taxa_df <- data.frame(tax_table(seed_phyloseq_decontam))
seed_taxa_df <- rownames_to_column(seed_taxa_df, var="ASVID")
seed_taxa_df$Keep <- seed_taxa_df$ASVID != "0232b56c81bb04fb94bdcfdf8b073ae1"
table(seed_taxa_df$Keep)
seed_taxa_df_keep <- filter(seed_taxa_df, seed_taxa_df$Keep == TRUE)
seed_taxa_list <- seed_taxa_df_keep$ASVID
seed_phyloseq_decontam <- prune_taxa(seed_taxa_list, seed_phyloseq_decontam)
...
seed_alpha$SampleID <- rownames(seed_alpha)
#seed_alpha
...
#ggsave("/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/alpha_patchwork_NEW.png", plot=richness_plots, width = 12, height=10, units="in")
...
seed_G2_C_WU_adonis
# NEW line         11 0.052020 0.34848 1.1851 0.0805 .
# NEW G2_treatment  2 0.009468 0.06343 1.1863 0.2251 
...
seed_G2_D_WU_adonis
# NEW  line  r2= 0.39347 F=1.4131 p=0.0032 **
# NEW G2_treatment  2 0.003200 0.04963 0.9804 0.4866 
...
seed_G2_N_WU_adonis
# NEW line         11 0.025649 0.35515 1.1534 0.2300
# NEW G2_treatment  2 0.002098 0.02905 0.5189 0.9616
...
#ggsave(filename="/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/seed_G2_3plot_WU_new.png", plot=seed_G2_3plot_WU, width=12, height=10, units="in")
...
fullG2_wunif_betadisper <- betadisper(seed_G2_wunifrac_dist, group=as.factor(seed_G2_WU_metadata$G1_treatment), type = "median", bias.adjust=TRUE)
fullG2_wunif_betadisper

wunif_permute <- permutest(fullG2_wunif_betadisper, pairwise=TRUE, permutations=9999)
wunif_permute

# no significance between G1 trt NEW:  Df=2 Sum Sq=0.001708 Mean Sq=0.00085393 F=1.4246   N.perm=9999 p=0.2132
# no significance between lines NEW:     35 0.031840 0.00090971 1.0388   9999 0.4541
...
proportion_data <- summed_data %>% mutate(proportion = if_else(line == "C10", (n_asv/83),
                                                        if_else(line == "C11", (n_asv/111),
                                                        if_else(line == "C12", (n_asv/105),
                                                        if_else(line == "C13", (n_asv/83),
                                                        if_else(line == "C15", (n_asv/74),
                                                        if_else(line == "C1B", (n_asv/79),
                                                        if_else(line == "C2", (n_asv/103),
                                                        if_else(line == "C2B", (n_asv/76),
                                                        if_else(line == "C4", (n_asv/113),
                                                        if_else(line == "C5", (n_asv/74),
                                                        if_else(line == "C6", (n_asv/105),
                                                        if_else(line == "C8", (n_asv/98),
                                                        if_else(line == "D10", (n_asv/82),
                                                        if_else(line == "D11", (n_asv/76),
                                                        if_else(line == "D13", (n_asv/112),
                                                        if_else(line == "D14", (n_asv/71),
                                                        if_else(line == "D15", (n_asv/86),
                                                        if_else(line == "D1B", (n_asv/122),
                                                        if_else(line == "D2B", (n_asv/108),
                                                        if_else(line == "D5", (n_asv/89),
                                                        if_else(line == "D6", (n_asv/90),
                                                        if_else(line == "D7", (n_asv/99),
                                                        if_else(line == "D8", (n_asv/72),
                                                        if_else(line == "D9", (n_asv/78),
                                                        if_else(line == "N10", (n_asv/71),
                                                        if_else(line == "N11", (n_asv/68),
                                                        if_else(line == "N13", (n_asv/82),
                                                        if_else(line == "N15", (n_asv/90),
                                                        if_else(line == "N1B", (n_asv/105),
                                                        if_else(line == "N2", (n_asv/121),
                                                        if_else(line == "N2B", (n_asv/101),
                                                        if_else(line == "N3", (n_asv/90),
                                                        if_else(line == "N3B", (n_asv/114),
                                                        if_else(line == "N5", (n_asv/109),
                                                        if_else(line == "N5B", (n_asv/130),
                                                        if_else(line == "N8", (n_asv/84), NA))))))))))))))))))))))))))))))))))))) %>% filter(intersects != "Total")

# calculate min, mean and max proportion for overlap taxa
overlap_prop <- proportion_data %>% filter(intersects == "G1_G2\nOverlap")
mean(overlap_prop$proportion) # 0.2821012 NEW
min(overlap_prop$proportion) # 0.1632653 NEW
max(overlap_prop$proportion) # 0.4444444 NEW
...
overlap_plot
#ggsave("/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/overlap_plot_new.png", overlap_plot, width=8, height=6, units="in")
...
violins_patchwork
ggsave("/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/overlap_plot_violins_new.png", violins_patchwork, width=13, height=6, units="in")
...
Upset_16S_all_label
# png(file="/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/overlap_upset_plot_all_new.png", width=9, height=5, units="in", res=300)
# Upset_16S_all_label
# dev.off()
...
rel_abund_plot <- ggplot(data=df_16S_upset_all, aes(x=n_gen, y=log)) + geom_boxplot(fill="gray") + xlab("Number of Generations") + ylab("Log Mean Relative Abundance") + my_theme + stat_summary(shape=15) + 
...
#ggsave(filename="/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/rel_abund_plot_new.png", plot = rel_abund_plot, width=6, height=5, units="in")
...
# 62 total taxa in at least one G1_G2 overlap
...
# 75 total taxa in at least one G1_G2 overlap
...
# 64 total taxa in at least one G1_G2 overlap
...
#ggsave(filename="/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/bubble_plot_69.png", bubble_plot_70, width=13.7, height=14, units="in")
...
#ggsave(filename="/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/bubble_plot_g0_new.png", bubble_plot_g0, width=2, height=14, units="in")
...
taxa_filter_core$OTU[2] <- "Seed"
taxa_filter_core$OTU[4] <- "Seed"
taxa_filter_core$taxaID <- seq(1,69, by=1)
taxa_filter_core$OTU[49] <- "Seed"
...
# venn diagrams ####
venn.diagram(x = list(control_taxa_list, drought_taxa_list, nutrient_taxa_list), filename="/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/treatment_taxa_venn_NEW.png", imagetype = "png", category.names = c("Control Lines\n(62 Total)", "Drought Lines\n(75 Total)", "Nutrient Lines\n(64 Total)"), fontfamily="sans", cat.fontfamily="sans", main="Taxa in G1-G2 Overlaps Shared Between Parent Lines", main.fontfamily = "sans", main.fontface="bold", disable.logging=TRUE, na="remove", width=9, height=9, units="in", cex=2.5, main.cex=2, cat.cex=1.8, col=c("#3399FF", "#FFCC00", "#18902B")) #, cat.col=c("black", "#3399FF", "#FF6600")
...
colors_32 <- c(colors_a, colors_b)
# Create the plot
barplot.genus.plot_all <- ggplot(data=df.barplot.genus_all, aes(x=line, y=Mean, fill=Genus)) + 
  scale_fill_manual(values=colors_32, labels=waiver()) + ylab("Mean Relative Abundance") + xlab("Plant Line") +
...
colors_34 <- c(colors_a, colors_b, colors_c[1], colors_c[5])
...
```{r compare_to_root_rhizo}
RR_rarefied <- readRDS(file="/Users/Abby/OneDrive - Michigan State University/multigen_analysis/root_rhizo_rarefiedphyloseq.rds")

#trim to 69 taxa of interest
root_rhizo_69taxa <- prune_taxa(taxa_order_nlines_filter, RR_rarefied)
root_rhizo_69taxa

root_rhizo_98taxa <- prune_taxa(all_line_taxa_list, RR_rarefied)
root_rhizo_98taxa


seed_root_rhizo_phyloseq <- merge_phyloseq(root_rhizo_98taxa, seed_phyloseq_decontam)

sample_data(seed_root_rhizo_phyloseq)
seed_root_rhizo_phyloseq <- prune_taxa(all_line_taxa_list, seed_root_rhizo_phyloseq)

merged_srr <- merge_samples(seed_root_rhizo_phyloseq, "Compartment")
count_16S_srr <- t(otu_table(merged_srr))
count_16S_srr[count_16S_srr > 0] <- 1
df_16S_srr <- as.data.frame(count_16S_srr)
rel.abu_srr <- data.frame(taxa_sums(seed_phyloseq_decontam))
rel.abu_srr$log <- log10(rel.abu_srr$taxa_sums.seed_phyloseq_decontam)
df_16S_upset_srr <-merge.data.frame(df_16S_srr, rel.abu_srr, by="row.names", all.x=TRUE)
Upset_16S_srr <- upset(df_16S_upset_srr,
                      nintersects = 50, #max number of combinations
                      order.by = "freq",  #putting highest numbered group first
  sets.x.label="ASVs per Compartment",text.scale=c(1.3,1.3,1.3,1,1.8,2.5),  #text
                      decreasing ="TRUE", mainbar.y.label = "Number of ASVs")
Upset_16S_srr

png(file="/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/upset_plot_srr.png", width=9, height=5, units="in", res=300)
Upset_16S_srr
dev.off()


df_16S_srr_add <- df_16S_srr
df_16S_srr_add$ASVID <- rownames(df_16S_srr_add)
df_16S_srr_add_list <- c(df_16S_srr_add$ASVID)

filter_69 <- df_16S_srr_add_list %in% taxa_order_nlines_filter 
df_16S_srr_add$in_69 <- filter_69
df_16S_srr_add <- filter(df_16S_srr_add, in_69 == TRUE)
df_16S_srr_add <- df_16S_srr_add[match(taxa_order_nlines_filter, df_16S_srr_add$ASVID),]

df_16S_srr_add$n_comparts <- df_16S_srr_add$rhizosphere + df_16S_srr_add$root + df_16S_srr_add$seed + df_16S_srr_add$`Source Soil`

df_16S_srr_add$in_soils <- if_else(df_16S_srr_add$n_comparts > 1, 1, 0)

taxa_filter 

taxa_filter$Present_in_RR <- df_16S_srr_add$in_soils

taxa_filter$RR <- "ASV_in_Soil"

bubble_plot_RR <- ggplot(data=taxa_filter, aes(x=RR, y=factor(asvID, level=taxa_order_nlines_filter))) + geom_point(aes(size=factor(Present_in_RR), color=factor(Present_in_RR)), shape=15) + scale_y_discrete(limits=rev) + scale_x_discrete(position = "top") + scale_color_manual(name = "ASV Present\nin Soils", values=c("white", "purple"), breaks=waiver(), labels=c("No", "Yes"))  + scale_size_manual(name = "ASV Present\nin Soils", values=c(0, 6), breaks = waiver(), labels=c("No", "Yes")) + theme(axis.text.y=element_blank(), axis.title.y = element_blank())
bubble_plot_RR

ggsave(filename="/Users/Abby/OneDrive - Michigan State University/multigen_analysis/seeds/bubble_plot_rhizo.png", bubble_plot_RR, width=2, height=14, units="in")

sample_data(root_rhizo_98taxa)
root_phylo <- root_rhizo_98taxa %>% ps_filter(Compartment == "root", .keep_all_taxa = F)
rhizo_phylo <- root_rhizo_98taxa %>% ps_filter(Compartment == "rhizosphere", .keep_all_taxa = F)
soil_phylo <- root_rhizo_98taxa %>% ps_filter(Compartment == "Source Soil", .keep_all_taxa = F)

root_otu <- data.frame(otu_table(root_phylo))
rhizo_otu <- data.frame(otu_table(rhizo_phylo))
soil_otu <- data.frame(otu_table(soil_phylo))

root_otu$ASVID <- rownames(root_otu) 
rhizo_otu$ASVID <- rownames(rhizo_otu) 
soil_otu$ASVID <- rownames(soil_otu) 

root_list <- c(root_otu$ASVID)
rhizo_list <- c(rhizo_otu$ASVID)
soil_list <- c(soil_otu$ASVID)
